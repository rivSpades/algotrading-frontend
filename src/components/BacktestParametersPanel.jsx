...
export default function BacktestParametersPanel({ backtest, title = 'Backtest parameters' }) {
  ...
  return (
    <div className="mb-6 bg-surface rounded-lg shadow border border-border p-4">
      ...
      <dl className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-2 text-sm">
        {backtest.name != null && String(backtest.name).trim() !== '' && (
          <>
            ...
            <dd className="text-ink font-medium sm:col-span-2">{backtest.name}</dd>
          </>
        )}
        <dt className="text-ink-tertiary">Train / test split</dt>
        ...
        {backtest.broker != null && backtest.broker !== '' && (
          <>
            <dt className="text-ink-tertiary">Broker (filter)</dt>
            <dd className="text-ink sm:col-span-2">{String(backtest.broker)}</dd>
          </>
        )}
        <dt className="text-ink-tertiary">Data window (stored)</dt>
        <dd className="text-ink sm:col-span-2">
          {fmtDate(backtest.start_date)} → {fmtDate(backtest.end_date)}
        </dd>
        <dt className="text-ink-tertiary">Hybrid VIX hedge</dt>
        <dd className="text-ink sm:col-span-2">{backtest.hedge_enabled ? 'On' : 'Off'}</dd>
        {backtest.hedge_enabled && (
          <>
            <dt className="text-ink-tertiary">Strategy-only baseline</dt>
            <dd className="text-ink sm:col-span-2">
        ...
        {backtest.hedge_enabled && hedgeKeys.length > 0 && (
          <>
            <dt className="text-ink-tertiary">Hedge config</dt>
            <dd className="text-ink sm:col-span-2 font-mono text-xs break-all">
      ...
      {sp && typeof sp === 'object' && Object.keys(sp).length > 0 && (
        <div className="mt-4 pt-4 border-t border-border">
          <h3 className="text-sm font-semibold text-ink mb-2">Strategy parameters</h3>
          <dl className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-1 text-sm">
            {Object.entries(sp).map(([k, v]) => (
              <div key={k} className="contents">
                <dt className="text-ink-tertiary capitalize">{k.replace(/_/g, ' ')}</dt>
                <dd className="text-ink sm:col-span-2">{String(v)}</dd>
              </div>
            ))}
          </dl>
        </div>
