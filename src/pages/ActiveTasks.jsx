  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-ink mb-2">Active Tasks</h1>
          <p className="text-ink-secondary">Monitor and manage currently executing tasks</p>
        </div>
        <button
          onClick={() => {
            loadActiveTasks();
      ...
      <div className="mb-8 bg-surface rounded-lg shadow p-6 border border-border">
        <h2 className="text-lg font-semibold text-ink mb-2">Broker queue</h2>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {brokerInfo?.error && (
            <div className="text-sm text-loss-ink">
              Error: {brokerInfo.error}
            </div>
          )}
          <button
            onClick={handlePurgeQueue}
            className="ml-auto px-4 py-2 bg-loss text-white rounded-lg hover:bg-loss transition-colors flex items-center gap-2"
      ...
      <div className="mb-8">
        ...
        {loading && activeTasks.length === 0 ? (
          <div className="bg-surface rounded-lg shadow p-8 text-center">
            <p className="text-ink-tertiary">Loading active tasks...</p>
          </div>
        ) : activeTasks.length === 0 ? (
          <div className="bg-surface rounded-lg shadow p-8 text-center">
            <Play className="w-12 h-12 text-ink-tertiary mx-auto mb-4" />
            <p className="text-ink-tertiary">No active tasks</p>
      ...
      <div>
        ...
        {historyLoading ? (
          <div className="bg-surface rounded-lg shadow p-8 text-center">
            <p className="text-ink-tertiary">Loading task history...</p>
          </div>
        ) : taskHistory.length === 0 ? (
          <div className="bg-surface rounded-lg shadow p-8 text-center">
            <Clock className="w-12 h-12 text-ink-tertiary mx-auto mb-4" />
            <p className="text-ink-tertiary">No task history</p>
            <p className="text-sm text-ink-tertiary mt-2">
              ...
              <table className="min-w-full divide-y divide-border">
                ...
                <tbody className="bg-surface divide-y divide-border">
                  {taskHistory.map((task) => (
                    <tr key={task.task_id} className="hover:bg-bg">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {task.success ? (
                          <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-profit-soft text-profit-ink">
                            <CheckCircle className="w-3 h-3" />
                            Completed
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-loss-soft text-loss-ink">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm text-ink-secondary max-w-md truncate">
                          {task.message || 'N/A'}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
...
function ActiveTaskCard({ task, onStop, formatTaskName, formatTimestamp, onTaskComplete }) {
  ...
  return (
    <motion.div
      ...
      className="bg-surface rounded-lg shadow p-6 border border-border"
    >
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="px-2 py-1 rounded-full text-xs font-medium bg-profit-soft text-profit-ink">
              {task.status === 'RUNNING' ? 'Running' : task.status || 'Running'}
            </span>
          </div>
          <p className="text-sm text-ink-tertiary mb-2">Task ID: {task.task_id}</p>
          {task.message && (
            <p className="text-sm text-ink-secondary mb-2">{task.message}</p>
          )}
          <div className="flex items-center gap-4 mt-3 text-sm text-ink-secondary">
            <div className="flex items-center gap-2">
              <div className="w-32 bg-surface-sunken rounded-full h-2">
                <div
            ...
            {task.worker && (
              <span className="text-ink-tertiary">Worker: {task.worker}</span>
            )}
          </div>
        </div>
        <button
          onClick={() => onStop(task.task_id)}
          className="ml-4 px-4 py-2 bg-loss text-white rounded-lg hover:bg-loss transition-colors flex items-center gap-2"
        >
          <Square className="w-4 h-4" />
          Stop
        </button>
